...
(function(){
  ...
  EP.canvas.on('mouse:up', function(){
    ...
    if (window.confirm('이곳의 워터마크나 글자를 지우겠습니까?')) {
      ...
      coverRectWithSurroundingColor(imgData, rect, w2, h2, 60);
  ...
  /* ============================================================
     3. 가장자리 블러(비네트 스타일) — 사용자가 고른 색이 가장자리로 갈수록 부드럽게
  ...
  // Esc로도 두 도구 모드를 빠져나올 수 있게 함(자동누끼는 여러 번 클릭하는 도구라 자동으로는
  // 안 꺼지므로, 다 쓴 뒤 빠져나오는 확실한 방법이 필요함)
  ...
  EP.exitImageToolModes = function(){
    if (isMagicWandMode) setMagicWandMode(false);
    if (isMarqueeEraseMode) setMarqueeEraseMode(false);
    // 혹시 위 두 모드 다 꺼진 상태인데도 어떤 이유로 skipTargetFind가 남아있는 경우까지 대비한 안전장치
    EP.canvas.skipTargetFind = false;
    EP.canvas.selection = true;
  };
})();
